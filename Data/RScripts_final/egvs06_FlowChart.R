...
diagrammai="digraph flowchart {
  ...
  node [fontname = Helvetica, shape = rectangle,style=rounded,fontface=bold,fontsize=30,fontcolor=black];{
    ...
    node [label='Terrain\n(8 quantities,\n1 set of coverage fractions)']; lsc6;
  }
  ...
  node [fontname = Helvetica, shape = rectangle,style=\"rounded,filled\",fontcolor=black,fillcolor=Azure]; {
    ...
    # specify which nodes are of the same 'rank' so that they'll be drawn at the same level
      {rank = same; level0 lsc1 lsc2 lsc3 lsc4 lsc5 lsc6 div}
      {rank = same; level1 climate hydroclim earthobs soilchem soiltxt l1_c01 l0_c02 l0_c03 l0_c04 l0_c05 l0_c06 l0_c07 l0_c08 l1_d01 l1_d02 l1_d03}
...
DiagrammeRsvg::export_svg(grViz(diagrammai)) |>
  charToRaw() |>
  rsvg::rsvg_png("EGV_FlowChartY.png", width = 2000)


DiagrammeRsvg::export_svg(grViz(diagrammai)) |>
  charToRaw() |>
  rsvg::rsvg_pdf("EGV_FlowChartY.pdf", width = 2000)


DiagrammeRsvg::export_svg(grViz(diagrammai)) |>
  charToRaw() |>
  rsvg::rsvg_svg("EGV_FlowChartY.svg", width = 2000)


DiagrammeRsvg::export_svg(grViz(diagrammai)) |>
  charToRaw() |>
  rsvg::rsvg_eps("EGV_FlowChartY.eps", width = 2000)
