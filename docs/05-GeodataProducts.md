...
from [LVM Open Data](#Ch04.06) were used to address the challenges (both buffered 
...
2. to calculate the topographic wetness index, terrain depressions without 
runoff were reviewed, allowing up to ten cell breaks in areas of lower 
resistance; the rest were filled in;

3. for additional security, the procedure of the second step was repeated to 
...
1. the base soil texture source was [Soil texture layer from the European Soil Database](#Ch04.07.02). 
This layer had to be reclassified to match the other layers, as this was not performed 
during preprocessing;

2. the layer from the first step was overlaid with the [Latvian Quarternary geology data](#Ch04.07.04) 
coded as numeric starting with 1;

3. the layer from the second step was overlaid with the [20th century topsoil data in Latvian farmland](#Ch04.07.03) 
coded as numeric starting with 1;

4. the layer from [Organic soils as modelled by the LVMI Silava](#Ch04.07.05) (presence-only) 
was overlaid with the [Organic soils as modelled by the University of Latvia](#Ch04.07.06) 
(presence-absence). After the overlay, it was classified as presence-only;

5. the layer from the third step was the overlaid with the layer from the fourth step and 
...
combined (in order of overlap):
    ...
    – [topographic map](#Ch04.04) layer `LandusA_COMB` values: "poligons_Augludarzs", 
    "poligons_Augļudārzs", "poligons_Sakņudārzs", "poligons_Ogulājs", "poligons_Ogulajs", 
    "poligons_Saknudarzs", coded as `420`;

    – [LAD database](#Ch04.02) rural information layer group (classes are 
    available [here](https://github.com/aavotins/HiQBioDiv_EGVs/blob/main/Data/Geodata/2024/LAD/KulturuKodi_2024.xlsx)) 
    “augļudārzi”, the result of which is coded as `420`.
...
class (in order of overlap):
    ...
    – [topographic map](#Ch04.04) layers `LandusA_COMB` classes: "poligons_Krūmājs", 
...
combined (in order of overlap):
    ...
    – [topographic map](#Ch04.04) layer `LandusA_COMB` classes: "poligons_Nec_purvs_sūnājs", 
    ...
    – land categories “21”, “22”, and “23” marked in the [State Forest Register](#Ch04.01), 
...
broad [agricultural codes (field - SDM_grupa_sakums)](https://github.com/aavotins/HiQBioDiv_EGVs/blob/main/Data/Geodata/2024/LAD/KulturuKodi_2024.xlsx) from [Rural Support Service's information on declared fields](#Ch04.02). Only cells 
...
landscapes using the function `egvtools::landscape_function`. To guard value coverage, 
...
landscapes using the function `egvtools::landscape_function`. To ensure value coverage, 
inverse distance weighted (power = 2) gap filling is incorporated; however,
there were no gaps to fill.
...
- farmland diversity class values prepared in [Overall landscape diversity](#Ch05.04.01);
