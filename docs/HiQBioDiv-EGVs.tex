...
Soil database \citep{esdac2}. Dataset decribing soil chemistry is derived from \href{https://esdac.jrc.ec.europa.eu/content/chemical-properties-european-scale-based-lucas-topsoil-data}{LUCAS
...
Soil database \citep{esdac2}. Dataset is available as \href{https://esdac.jrc.ec.europa.eu/content/european-soil-database-v2-raster-library-1kmx1km}{European Soil Database v2 Raster Library 1kmx1km}. There
...
cloudiness ≤35, allowing for filtering and various aggregations \citep{DynWorld}.

DW input information - raster layer for each season in each year - prepared on
the Google Earth Engine platform \citep{GEEpaper} using
...
loss from 2001 to the current version, which is updated annually \citep{theGFW}. The
...
with trees, where tree cover (at least 5 m high) is at least 10\% \citep{PALSARForest}.
...
30 arc second global downscaled climate data set \citep{CHELSA}. The temperature algorithm
...
that its predictions of precipitation patterns are better \citep{CHELSA}. Data
...
created for biodiversity analysis at 1 km resolution \citep{HydroClim}. Dataset
contains many different variables along the HydroSHEDS river
network \citep{HydroSheds}, including upstream climate recalculated from
worldclim \citep{worldclim_hijmans}. We downloaded (to \passthrough{\lstinline!Geodata/2024/HydroClim/!})
...
we need continuously covered raster surfaces. For necessary geoprocessing to create such surfaces, we downloaded also HydroBASINS \citep{HydroBasins} \href{https://www.hydrosheds.org/products/hydrobasins}{dataset} to \passthrough{\lstinline!Geodata/2024/HydroClim/!}.
...
platform \citep{GEEpaper} for various large-scale pre-processing and analysis. We use
...
\item
  ...
  search for and fill in terrain depressions \citep{WangLiu2006};
\item
